...
package com.ys.customwidget.CustomTabLayout;
...
public class CustomTabLayout extends HorizontalScrollView {
    ...
    private boolean mNeedSwitchAnimation = true;
    ...
    float mTabSelectTextSize = 0;
    Typeface mTabSelectTypeface;
    ...
    class TabView extends LinearLayout {
        ...
        public void setSelected(final boolean selected) {
            ...
            if (mTextView != null) {
                ...
                if (selected) {
                    if (mTabSelectTextSize > 0) {
                        mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTabSelectTextSize);
                    }
                    if (mTabSelectTypeface != null) {
                        mTextView.setTypeface(mTabSelectTypeface);
                    }
                } else {
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private float[] radiusArray = {dpToPx(2), dpToPx(2), dpToPx(2), dpToPx(2), dpToPx(2), dpToPx(2), dpToPx(2), dpToPx(2)};
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                if (mSelectedIndicatorWidth == -1 || isIndicatorWidthWrapContent()) {
...
//                            getHeight(), mSelectedIndicatorPaint);
                    ...
                    Path path = new Path();
                    path.addRoundRect(new RectF(left, getHeight() - mSelectedIndicatorHeight, right, getHeight()), radiusArray, Path.Direction.CW);
...
//                            mIndicatorRight, getHeight(), mSelectedIndicatorPaint);
                    ...
                    Path path = new Path();
                    path.addRoundRect(new RectF(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight, mIndicatorRight, getHeight()), radiusArray, Path.Direction.CW);
                    canvas.drawPath(path, mSelectedIndicatorPaint);
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            if (customTabLayout != null) {
                ...
                if (customTabLayout.mNeedSwitchAnimation) {
                    if (mScrollState == SCROLL_STATE_SETTLING && mPreviousScrollState == SCROLL_STATE_IDLE) {
                        customTabLayout.mTabStrip.mIsDragged = false;
                    } else {
                        customTabLayout.mTabStrip.mIsDragged = true;
                        if (customTabLayout.mTabStrip.mSelectedPosition + 1 < customTabLayout.mTabStrip.getChildCount()) {
                            float offset = (customTabLayout.mTabStrip.getChildAt(customTabLayout.mTabStrip.mSelectedPosition).getMeasuredWidth()
                                    - customTabLayout.mTabStrip.getWrapContentIndicatorWidth(customTabLayout.mTabStrip.mSelectedPosition)
                            ) / 2.0f;
                            float leftX = customTabLayout.mTabStrip.getChildAt(customTabLayout.mTabStrip.mSelectedPosition).getLeft() + offset;
                            float rightX = leftX + customTabLayout.mTabStrip.getWrapContentIndicatorWidth(customTabLayout.mTabStrip.mSelectedPosition);

                            float nextOffset = (customTabLayout.mTabStrip.getChildAt(customTabLayout.mTabStrip.mSelectedPosition + 1).getMeasuredWidth()
                                    - customTabLayout.mTabStrip.getWrapContentIndicatorWidth(customTabLayout.mTabStrip.mSelectedPosition + 1)
                            ) / 2.0f;
                            float nextLeftX = customTabLayout.mTabStrip.getChildAt(customTabLayout.mTabStrip.mSelectedPosition).getLeft()
                                    + customTabLayout.mTabStrip.getChildAt(customTabLayout.mTabStrip.mSelectedPosition).getWidth() + nextOffset;
                            float nextRightX = nextLeftX +
                                    customTabLayout.mTabStrip.getWrapContentIndicatorWidth(customTabLayout.mTabStrip.mSelectedPosition + 1);
                            customTabLayout.mTabStrip.left = leftX + (nextLeftX - leftX) * mStartInterpolator.getInterpolation(positionOffset);
                            customTabLayout.mTabStrip.right = rightX + (nextRightX - rightX) * mEndInterpolator.getInterpolation(positionOffset);
                            ViewCompat.postInvalidateOnAnimation(customTabLayout.mTabStrip);
                        }
                    }
                }
            }
    ...
    public boolean isIndicatorWidthWrapContent() {
        return mTabStrip.isIndicatorWidthWrapContent();
    }
    ...
    public void setTabSelectTextSize(float size) {
    ...
    public void setTabSelectTypeface(Typeface typeface) {
        this.mTabSelectTypeface = typeface;
        if (mSelectedTab != null) {
            mSelectedTab.select();
        }
    }
